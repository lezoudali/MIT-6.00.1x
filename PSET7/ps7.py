...
class WordTrigger(Trigger):
    ...
    def isWordIn(self, text):
        for p in string.punctuation:
            text = text.replace(p, ' ') if p == "'" else text.replace(p, '')
        return True if self.word.lower() in text.lower().split(' ') else False
...
class PhraseTrigger(Trigger):
    ...
    def __init__(self, phrase):
        self.phrase=phrase
    def evaluate(self, story):
       return True if self.phrase in (story.getSubject() or story.getTitle() or story.getSummary()) else False
